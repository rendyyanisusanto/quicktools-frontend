<template>
  <ToolPageLayout
    title="PNG ke JPG Online Gratis"
    description="Konversi gambar PNG menjadi file JPG yang lebih kecil secara instan menggunakan teknologi berbasis peramban tanpa mengupload data Anda."
  ...
  >
    ...
    <ToolActionCard>
      ...
      <ImageUploadZone
        v-if="state === 'idle'"
        hint="Pilih 1 file PNG maksimal 20MB"
        accept="image/png"
        :multiple="false"
        ...
        <div class="bg-gray-50 border border-gray-100 rounded-lg p-5">
          ...
          <div class="flex flex-col sm:flex-row items-center justify-center gap-6 mb-5">
            ...
            <div class="w-full sm:w-1/3 flex flex-col items-center">
               <div class="w-24 h-24 bg-gray-200 border border-gray-300 rounded flex items-center justify-center overflow-hidden shadow-sm relative mb-3 bg-checkbox-pattern">
                  <img :src="previewSrc" class="max-w-full max-h-full object-contain drop-shadow-md" />
               </div>
               <span class="bg-blue-100 border border-blue-200 text-blue-800 text-xs font-bold px-3 py-1 rounded-full uppercase">
                 .PNG
               </span>
               <p class="text-xs text-gray-500 mt-2 truncate w-full px-2 text-center" :title="originalFile.name">{{ originalFile.name }}</p>
            ...
            <div class="w-full sm:w-1/3 flex flex-col items-center">
               ...
               <p v-if="convertedBlob" class="text-xs text-gray-500 mt-2 truncate w-full px-2 text-center">Hasil Konversi</p>
               <p v-if="convertedBlob" class="text-[10px] text-gray-400 font-medium text-green-600">{{ formatSize(convertedBlob.size) }} (Hemat)</p>
               <p v-else class="text-[10px] text-gray-400 font-medium mt-2">Menunggu diproses</p>
            </div>
          </div>

          <!-- JPEG background warning -->
          <div class="bg-yellow-50 text-yellow-800 text-xs p-3 rounded border border-yellow-200 text-center">
            <strong>Catatan:</strong> Jika PNG Anda memiliki area transparan, akan diisi dengan latar belakang berwarna putih pada hasil JPG.
          </div>
    ...
    <section class="mt-10 prose prose-gray max-w-none">
      <h2 class="text-xl font-bold text-gray-900 mb-3">Mengapa Mengonversi ke JPG?</h2>
      <p class="text-gray-500 text-sm leading-relaxed">
        Banyak sistem dari Pemerintah, Perusahaan, atau Kampus yang menolak atau gagal membaca foto dalam format modern seperti PNG atau WEBP. Mereka secara ketat meminta format klasik `.jpg` atau `.jpeg`. Selain itu, merubah format dokumen dari tipe PNG ke JPG bisa memangkas ukuran file hingga sebagian besar ukurannya.
      </p>
    </section>

    <!-- HOW TO USE -->
    <section class="mt-10">
      <h2 class="text-xl font-bold text-gray-900 mb-4">Cara Kerja PNG ke JPG</h2>
      <ol class="space-y-3">
        <li
...
// Metadata
const toolData = getImageTool('png-to-jpg');
...
function onFileSelected(selectedFiles) {
  ...
  if (f.type !== 'image/png') {
    alert('Harap masukkan file gambar berformat PNG.');
    return;
  }
...
function processConversion() {
  ...
  setTimeout(() => {
    try {
      ...
      // Fill background manually to avoid black backgrounds on transparent PNGs 
...
function downloadImage() {
  ...
  a.download = `${rawName}-converted.jpg`;
